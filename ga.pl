...
my $infile 				= "./usd_brl_big_diff1.txt"; 		# indicators results input file
...
sub subjectFitness_avg_n {
	...
	my $usedind = NumberOfSetBits($indn);
	my $theFitness = 0.00;
	...
	for my $trow (0..$#training){
		...
		$bitMatch = ($indn & $training[$trow][0]);
		$numBitsMatch = NumberOfSetBits($bitMatch);
		$avg = $numBitsMatch / $usedind;
		...
		$theFitness += ($training[$trow][$indsize] != $myprediction);
	}
	return $theFitness;
}
...
sub BRUTE_FORCE {
	...
	while ($first < $limit) {
		...
		my $thd = async {
			...
			for (my $indn = $first; $indn <= $last; $indn++) {
				my $fit = subjectFitness_avg_n($indn);
				if ($bestFit > $fit) {
					$bestFit = $fit;
					$bestInd = $indn;
					my $indnum = NumberOfSetBits($indn);
					if ($bestIndNum > $indnum) {
						$bestIndNum = $indnum;
					}
				}
			}
...
sub prediction_avg {
	...
	for my $col (0..($indsize-1)){
		...
		#print "$indSum += ( @{$arInd}[$col] * @{$arTest}[$col] );\n";
